...
export class Renderer {
  ...
  async render(
    ...
    // Prototype toggles; every value is 0/off unless its experiment is enabled.
    const experiments = state.experiments;
    const strands = experiments?.strands.on ? experiments.strands : undefined;
    if (strokes.length)
      this.ribbonSetup(state.mode === "B", {
        displacePx: experiments?.displace.on
          ? ((Math.min(iw, ih) * experiments.displace.amount) / 100) * scale
          : 0,
        edgeAmount: experiments?.edgeWidth.on ? experiments.edgeWidth.amount : 0,
        edgeTexel: 3 * scale,
        shadeAmount: experiments?.shade.on ? experiments.shade.amount : 0,
      });
    ...
    for (const [index, stroke] of strokes.entries()) {
      ...
      if (samples.length > 1) {
        const verts = ribbonMesh(samples, scaled, state.mode, strands);
  ...
  private ribbonSetup(
    ...
    g.uniform1f(this.location(p, "displacePx"), experiments.displacePx);
    g.uniform1f(this.location(p, "edgeAmount"), experiments.edgeAmount);
    g.uniform1f(this.location(p, "edgeTexel"), experiments.edgeTexel);
    g.uniform1f(this.location(p, "shadeAmount"), experiments.shadeAmount);
