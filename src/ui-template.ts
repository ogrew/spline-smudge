...
<main><aside><fieldset id="controls"><section><div class="section-title">01 <h2>カラーピック</h2></div><div class="modes"><button id="mode-a" aria-pressed="true" title="始点の採取線で採った色の並びを、帯全体にのばす"><b>A</b><span>のばす<small>(STRETCH)</small></span></button><button id="mode-b" aria-pressed="false" title="各点で採った色を、点のあいだでつないで補間する"><b>B</b><span>つなぐ<small>(BLEND)</small></span></button><button id="mode-c" aria-pressed="false" title="写真に置いた採取経路をたどりながら色を採る"><b>C</b><span>たどる<small>(TRACE)</small></span></button></div><div id="mix-settings" hidden><label class="range-label" for="mix-mode">色補間</label>${select(
  "mix-mode",
  mixModes,
)}<div id="mix-spin-row" hidden>${range("mix-spin", "色相の回転数", -2, 2, 1)}</div></div></section>
<section><div class="section-title">02 <h2>スプライン</h2></div><label class="sr-only" for="kind">作品全体のスプライン</label>${select(
  "kind",
  kinds,
)}<div id="stroke-list" class="stroke-list" aria-label="ストローク一覧"></div><div class="stroke-actions"><button id="stroke-add">＋ 追加</button><button id="stroke-copy">複製</button><button id="stroke-delete">削除</button><button id="stroke-up" title="手前へ" aria-label="線を手前へ">↑</button><button id="stroke-down" title="奥へ" aria-label="線を奥へ">↓</button></div><div id="tcb">${range("tension", "Tension / 張り", -1, 1, 0.01)}${range("continuity", "Continuity / つながり", -1, 1, 0.01)}${range("bias", "Bias / 偏り", -1, 1, 0.01)}</div>${range("width", "基本の太さ", 1, 160, 1)}<div class="selected"><span id="selected-name">点を選択してください</span>${range("factor", "この点の太さ", 0, 10, 0.05)}<button id="factor-reset" class="factor-reset">リセット</button></div><div class="button-row"><button id="sample">ランダムな曲線</button><button id="clear">点をクリア</button></div></section>
...
<section><div class="section-title">04 <h2>オプション</h2></div><label class="option-toggle"><input id="reaction" type="checkbox">写真で帯を変形</label><div id="reaction-settings" class="option-settings" hidden><label class="sr-only" for="reaction-mode">変形アルゴリズム</label>${select(
  "reaction-mode",
  reactionModes,
)}<div id="reaction-displace">${range("displace-amount", "変位量（短辺比）", -20, 20, 0.5)}</div><div id="reaction-edge">${range("edge-amount", "効き", -1, 1, 0.05)}</div></div><label class="option-toggle"><input id="shade" type="checkbox">フェイク3D</label><div id="shade-settings" class="option-settings" hidden>${range("shade-amount", "強さ", 0, 1, 0.05)}</div><label class="option-toggle"><input id="kasure" type="checkbox">習字のかすれ</label><div id="kasure-settings" class="option-settings" hidden>${range("kasure-amount", "強さ", 0, 1, 0.05)}${range("kasure-grain", "毛筋の間隔", 2, 48, 1)}</div></section>
