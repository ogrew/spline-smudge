...
<section><div class="section-title">04 <h2>オプション</h2></div><label class="option-toggle"><input id="reaction" type="checkbox">写真で帯を変形</label><div id="reaction-settings" class="option-settings" hidden><label class="sr-only" for="reaction-mode">変形アルゴリズム</label>${select(
  "reaction-mode",
  reactionModes,
)}<div id="reaction-displace">${range("displace-amount", "変位量（短辺比）", -20, 20, 0.5)}</div><div id="reaction-edge">${range("edge-amount", "効き", -1, 1, 0.05)}</div></div><label class="option-toggle"><input id="shade" type="checkbox">フェイク3D</label><div id="shade-settings" class="option-settings" hidden>${range("shade-amount", "強さ", 0, 1, 0.05)}</div><label class="option-toggle"><input id="kasure" type="checkbox">習字のかすれ</label><div id="kasure-settings" class="option-settings" hidden>${range("kasure-amount", "強さ", 0, 1, 0.05)}${range("kasure-grain", "毛筋の間隔", 2, 48, 1)}</div><label class="option-toggle"><input id="glow" type="checkbox">光のにじみ</label><div id="glow-settings" class="option-settings" hidden>${range("glow-width", "ひろがり", 0.1, 4, 0.05)}${range("glow-spread", "変化", -1, 1, 0.05)}${range("glow-core", "芯の輝き", 0, 1, 0.05)}</div></section>
