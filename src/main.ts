...
<section><div class="section-title">05 <h2>実験</h2></div><p class="note">評価用の試作。作品全体に効き、取り消しに対応します。</p><label class="exp-toggle"><input id="exp-displace" type="checkbox">輝度ディスプレイスメント</label>${range("exp-displace-amount", "変位量（短辺比）", -20, 20, 0.5)}<label class="exp-toggle"><input id="exp-edge" type="checkbox">エッジで幅を変調</label>${range("exp-edge-amount", "効き", -1, 1, 0.05)}<label class="exp-toggle"><input id="exp-strands" type="checkbox">ストランド分割</label>${range("exp-strand-count", "本数", 2, 24, 1)}${range("exp-strand-wobble", "乱れ", 0, 1, 0.05)}<label class="exp-toggle"><input id="exp-flow" type="checkbox">フロー場トレース（クリックで線を生成）</label>${range("exp-flow-length", "トレース長（長辺比）", 10, 90, 1)}<label class="exp-toggle"><input id="exp-shade" type="checkbox">フェイク3D</label>${range("exp-shade-amount", "強さ", 0, 1, 0.05)}</section></fieldset></aside>
...
function sync() {
  ...
  const values: Record<string, [number, string]> = {
    ...
    "exp-displace-amount": [
      experiments().displace.amount,
      `${experiments().displace.amount.toFixed(1)}%`,
    ],
    "exp-edge-amount": [
      experiments().edgeWidth.amount,
      experiments().edgeWidth.amount.toFixed(2),
    ],
    "exp-strand-count": [
      experiments().strands.count,
      `${experiments().strands.count} 本`,
    ],
    "exp-strand-wobble": [
      experiments().strands.wobble,
      experiments().strands.wobble.toFixed(2),
    ],
    "exp-flow-length": [
      experiments().flow.length,
      `${Math.round(experiments().flow.length)}%`,
    ],
    "exp-shade-amount": [
      experiments().shade.amount,
      experiments().shade.amount.toFixed(2),
    ],
  };
  for (const [id, on, sliders] of [
    ["exp-displace", experiments().displace.on, ["exp-displace-amount"]],
    ["exp-edge", experiments().edgeWidth.on, ["exp-edge-amount"]],
    [
      "exp-strands",
      experiments().strands.on,
      ["exp-strand-count", "exp-strand-wobble"],
    ],
    ["exp-flow", experiments().flow.on, ["exp-flow-length"]],
    ["exp-shade", experiments().shade.on, ["exp-shade-amount"]],
  ] as const) {
    $<HTMLInputElement>(id).checked = on;
    for (const slider of sliders) $<HTMLInputElement>(slider).disabled = !on;
  }
...
const changes: Record<string, (v: number) => void> = {
  ...
  "exp-displace-amount": (v) => (experiments().displace.amount = v),
  "exp-edge-amount": (v) => (experiments().edgeWidth.amount = v),
  "exp-strand-count": (v) =>
    (experiments().strands.count = Math.max(2, Math.round(v))),
  "exp-strand-wobble": (v) => (experiments().strands.wobble = v),
  "exp-flow-length": (v) => (experiments().flow.length = v),
  "exp-shade-amount": (v) => (experiments().shade.amount = v),
};
const currentValues: Record<string, () => number> = {
  ...
  bias: () => stroke().bias,
  "exp-displace-amount": () => experiments().displace.amount,
  "exp-edge-amount": () => experiments().edgeWidth.amount,
  "exp-strand-count": () => experiments().strands.count,
  "exp-strand-wobble": () => experiments().strands.wobble,
  "exp-flow-length": () => experiments().flow.length,
  "exp-shade-amount": () => experiments().shade.amount,
};
const experimentToggles: Record<string, (on: boolean) => void> = {
  "exp-displace": (on) => (experiments().displace.on = on),
  "exp-edge": (on) => (experiments().edgeWidth.on = on),
  "exp-strands": (on) => (experiments().strands.on = on),
  "exp-flow": (on) => (experiments().flow.on = on),
  "exp-shade": (on) => (experiments().shade.on = on),
};
for (const [id, apply] of Object.entries(experimentToggles))
...
$("stage").addEventListener("pointerdown", (event) => {
  ...
  if (space || event.button === 1) {
    ...
    if (existing) {
      ...
      sync();
    } else if (inside(p) && event.detail < 2 && experiments().flow.on) {
      // Flow tracing replaces the whole active stroke with a structure-following curve.
      history.push(state);
      clickAddedPoint = null;
      flowField ??= buildFlowField(source, iw, ih);
      const length = (Math.max(iw, ih) * experiments().flow.length) / 100;
      const trace = traceFlow(flowField, p, iw, ih, length);
      const count = Math.min(
        14,
        Math.max(4, Math.round(length / (Math.max(iw, ih) * 0.09))),
      );
      const s = stroke();
      s.points = tracePoints(trace, count).map((q) => ({
        ...q,
        id: crypto.randomUUID(),
        factor: 1,
        source: { angle: s.source.angle, length: s.source.length },
      }));
      selected = s.points[Math.floor(s.points.length / 2)]?.id ?? null;
      if (s.points[0]) {
        s.source.x = s.points[0].x;
        s.source.y = s.points[0].y;
      }
      requestRender();
